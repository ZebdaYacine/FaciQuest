...
class Data {
  Map<String, Set<Map<String, dynamic>>> provinces = {};

  

  Map<String, Set<Map<String, dynamic>>> groupByWilaya() {
    if (provinces.isNotEmpty) return provinces;
    // Create a map where the key is wilaya_name, and the value is a list of communes
    final Map<String, Set<Map<String, dynamic>>> grouped = {};

    for (var commune in algeria_cites) {
      final wilayaName = commune['wilaya_name'] as String;

      // If the wilaya_name doesn't exist in the map, add an empty list
      if (!grouped.containsKey(wilayaName)) {
        grouped[wilayaName] = {};
      }

      // Add the commune to the correct wilaya_name group
      grouped[wilayaName]!.add(commune);
    }
